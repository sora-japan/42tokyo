...
| `get_next_line.c` | `grow` / `compact` / `append` / `fill_save` | `static`（外から見えない） |
| | `get_next_line` | 公開 |
| `get_next_line_utils.c` | `nl_pos` / `reset_save` / `str_line` / `consume` | 公開（別ファイルから呼ぶため） |

Norm の「1ファイル5関数まで」に対して **5関数 / 4関数**。`get_next_line.c` は上限ちょうどなので、これ以上関数を足すなら `utils.c` 側へ置く。
...
**「文字列」ではなく「バイト列＋位置＋長さ＋容量」で持っている**のがこの実装の肝。理由は4つ:

1. **長さを数え直さなくていい** — 文字列だと連結のたびに先頭から `\0` を探す必要がある。
2. **`\0` が混ざっても壊れない** — 途中に `\0` があっても区切り文字を見失わない（バイナリは subject 上は未定義動作だが、壊れないに越したことはない）。
3. **`cap` を分離できる** — 「必要な量」と「確保済みの量」を分けることで、**容量が足りているときは malloc を一切しない**。
4. **`off` を分離できる** — 行を返したあと、残りをバッファ先頭へコピーし直さずに済む。**`BUFFER_SIZE` が大きいときの性能はこれで決まる**（§4「なぜ前詰めをやめたのか」参照）。

`off`・`len` は「有効データの先頭から数えた相対位置」で統一している。`nl_pos` の戻り値も `fill_save` の `start` も相対位置なので、途中で `compact()` が走って `off` が変わっても値が無効にならない。
...
### `consume()` — 開始位置を進めるだけ（malloc もコピーもしない）
...
数を2つ動かすだけの **O(1)**。前方の消費済み領域は、追記が必要になったときに `append` が `compact()` を呼んで回収する（`read` 1回につき高々1度）。

**ここは一度失敗している箇所。** 当初は「残りをバッファ先頭へ詰め直す」実装だった。malloc は増えないので一見よさそうだが、`BUFFER_SIZE` が大きいと1回目の read でファイル全体がバッファに載るため、**1行返すたびに残り全部をコピー**することになり `O(行数 × 残りバイト数)` に膨らんでいた（§4 に実測）。
...
**大きい側で落ちる実装が多い理由**（聞かれたら差をつけられる点）: 一時バッファを `char buf[BUFFER_SIZE];` とスタックに宣言すると、`BUFFER_SIZE=10000000` で 10MB をスタックに取ろうとして**スタックオーバーフローで即クラッシュ**する（macOS の既定スタックは 8MB）。実測でも素朴版は SIGSEGV で落ちた。本実装は `fill_save` で `malloc(BUFFER_SIZE)` とヒープに取っているので、`10000000` でも `leaks` 0 で完走する。

| 検証した値 | 結果 |
| --- | --- |
| `1` / `2` / `42` / `9999` | 全エッジケースで出力一致・リーク 0 |
| `100000` / `1000000` / `10000000` | 同上（出力・リークとも同一） |
...
読み取り単位が行長を超える（`9999` 以降）と、どちらも `read` 1回で済むので差は消える。**差が出るのは `BUFFER_SIZE` が小さいときだけ**で、そこで素朴な実装は `BUFFER_SIZE` に反比例して爆発する。

`10000000` で素朴版が落ちているのは別の理由（一時バッファを `char buf[BUFFER_SIZE]` とスタックに取ると 10MB でスタックオーバーフローする）。本実装は `malloc(BUFFER_SIZE)` でヒープに取るので生き残る → §7 参照。

### Q. なぜ「残りを前詰め」ではなく「`off` を進める」のか？（**最重要・実測あり**）

前詰めは1行返すたびに残り全部をコピーする。`BUFFER_SIZE` が大きいと1回目の read でファイル全体がバッファに載るので、行数 `M` × 残りバイト数 `N` で **`O(N·M)`** になる。

**実測（20万行 2.3MB のファイルを読み切るまでの時間）**

| `BUFFER_SIZE` | 前詰め方式（旧） | `off` 方式（現在） |
| --- | --- | --- |
| `1` | 0.90 秒 | 0.88 秒 |
| `42` | 0.24 秒 | 0.22 秒 |
| `9999` | 1.25 秒 | 0.19 秒 |
...
行数を2倍にすると旧方式は時間が約4倍（10000行 0.52秒 → 20000行 2.25秒 → 40000行 9.32秒）。二次関数的。

**なぜ気付きにくかったか**: 小さい `BUFFER_SIZE` では持ち越しが数十バイトしかないので表面化せず、小さいファイルでも表面化しない。**「大きい `BUFFER_SIZE`」と「多い行数」が揃ったときだけ**出る。subject が `10000000` を例示している以上、テスターに踏まれる可能性が高かった。

**回収はどうする**: `append` の冒頭で `off != 0` のときだけ `compact()` が先頭へ寄せる。`read` 1回につき高々1度、寄せる量もそのときの持ち越し分だけなので、全体では入力サイズに比例するコストに収まる。
...
| お題 | 変更点 |
| --- | --- |
| **区切り文字を `\n` 以外にする** | **対応済み**。`nl_pos` が `LINE_DELIMITER` を見ているので、ヘッダの `'\n'` を `'A'` に変える1文字、あるいは `-D "LINE_DELIMITER='A'"` を足すだけでソース無変更。「区切り文字と比較しているのは実装中1か所だけ」と `grep -n "LINE_DELIMITER" *.c` を見せて説明する |
| 返す行から区切り文字を取り除く | `str_line` に `copy` 変数を足し、`*len`（`consume` に渡す＝捨てる長さ）は `pos + 1` のまま、コピー長だけ `pos` にする。**`*len` ごと `pos` にすると区切り文字が残って空文字列の無限ループになる**（実測済み） |
...
- 一時読み込みバッファは **`malloc(BUFFER_SIZE)` でヒープに取る**。`char buf[BUFFER_SIZE];` とスタックに取る実装は `BUFFER_SIZE=10000000` でスタックオーバーフロー（実測で SIGSEGV）。subject が `10000000` を例示しているのはここを踏ませるため。
- `data` は `\0` 終端していない。長さは必ず `len` で判断する。`strlen` 的な発想で触ると壊れる。
- `data` の有効データは先頭から始まるとは限らない。読むときは必ず **`s->data[s->off + i]`**。`nl_pos` と `str_line` の2か所だけがこの添字を使う。
- `consume` の後も `cap` は減らない（＝メモリは確保したまま）。これは意図的な再利用。「解放し忘れでは？」と聞かれたら「次の行で再利用するため。`NULL` を返すときに `reset_save` で必ず解放している」と答える。
- `t_save` にフィールドを足したら **`reset_save` にも必ず追加する**。ここが初期状態への復帰を一手に引き受けているため、書き漏らすと fd を開き直したときだけ壊れる（再現しにくいバグになる）。
